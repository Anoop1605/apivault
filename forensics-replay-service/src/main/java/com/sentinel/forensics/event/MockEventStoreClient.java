...
public class MockEventStoreClient implements EventStoreClient {
    ...
    public List<EventDTO> fetchEvents(UUID sessionId) {
        if (SESSION_NORMAL.equals(sessionId))
            return normalSession();
        if (SESSION_VIOLATION.equals(sessionId))
            return violationSession();
        if (SESSION_ATTACK.equals(sessionId))
    ...
    private List<EventDTO> normalSession() {
        return new ArrayList<>(List.of(
                event(SESSION_NORMAL, 1707825600000000000L, "LOGIN", "ALLOW", "ALLOW_ALL", "GET", "/auth/login",
                        "user-001", 0.1f),
                event(SESSION_NORMAL, 1707825601000000000L, "ACCESS", "ALLOW", "ALLOW_GET", "GET", "/api/users",
                        "user-001", 0.1f),
                event(SESSION_NORMAL, 1707825602000000000L, "ACCESS", "ALLOW", "ALLOW_GET", "GET", "/api/payments",
                        "user-001", 0.1f),
                event(SESSION_NORMAL, 1707825603000000000L, "LOGOUT", "ALLOW", "ALLOW_ALL", "GET", "/auth/logout",
                        "user-001", 0.1f)));
    }
    ...
    private List<EventDTO> violationSession() {
        return new ArrayList<>(List.of(
                event(SESSION_VIOLATION, 1707825700000000000L, "LOGIN", "ALLOW", "ALLOW_ALL", "POST", "/auth/login",
                        "user-002", 0.2f),
                event(SESSION_VIOLATION, 1707825701000000000L, "ACCESS", "ALLOW", "ALLOW_GET", "GET",
                        "/api/admin/users", "user-002", 0.3f),
                event(SESSION_VIOLATION, 1707825702000000000L, "ACCESS", "BLOCK", "BLOCK_DELETE", "DELETE",
                        "/api/payments/txn-9921", "user-002", 0.5f),
                event(SESSION_VIOLATION, 1707825703000000000L, "ACCESS", "BLOCK", "BLOCK_DELETE", "DELETE",
                        "/api/payments/txn-9921", "user-002", 0.6f),
                event(SESSION_VIOLATION, 1707825704000000000L, "LOGOUT", "ALLOW", "ALLOW_ALL", "POST", "/auth/logout",
                        "user-002", 0.2f)));
    }
    ...
    private List<EventDTO> attackSession() {
        return new ArrayList<>(List.of(
                event(SESSION_ATTACK, 1707825800000000000L, "ACCESS", "ALLOW", "ALLOW_GET", "GET", "/api/users", null,
                        0.4f),
                event(SESSION_ATTACK, 1707825800100000000L, "ACCESS", "ALLOW", "ALLOW_GET", "GET", "/api/admin", null,
                        0.6f),
                event(SESSION_ATTACK, 1707825800200000000L, "ACCESS", "ALLOW", "ALLOW_GET", "GET",
                        "/api/payments?id=1 OR 1=1", null, 0.7f),
                event(SESSION_ATTACK, 1707825800300000000L, "ATTACK", "BLOCK", "BLOCK_ATTACK", "DELETE",
                        "/api/admin/users/delete-all", null, 0.85f),
                event(SESSION_ATTACK, 1707825800400000000L, "ATTACK", "BLOCK", "BLOCK_ATTACK", "POST",
                        "/api/admin/roles/assign", null, 0.85f),
                event(SESSION_ATTACK, 1707825800500000000L, "ATTACK", "BLOCK", "BLOCK_DELETE", "DELETE",
                        "/api/payments/delete-all", null, 0.9f)));
    }
    ...
    private EventDTO event(UUID sessionId, long timestampNs,
        ...
        e.setEventType(com.sentinel.shared.enums.EventType.valueOf(eventType));
        e.setDecision(com.sentinel.shared.enums.Decision.valueOf(decision));
        e.setPolicyRuleId(policyRuleId);
        e.setPolicyRuleVersion(1);
        e.setHttpMethod(httpMethod);
        e.setEndpoint(endpoint);
        e.setUserId(userId);
        e.setRiskScore((double) riskScore);
