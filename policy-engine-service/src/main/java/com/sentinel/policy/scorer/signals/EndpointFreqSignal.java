 ...
 *
 * Detects when a single user hits the SAME endpoint an abnormal number of times.
 ...
 * Acceptable because the scorer recalibrates within seconds of traffic resuming.
 */
@Component
public class EndpointFreqSignal implements RiskSignal {
     ...
     */
    private final ConcurrentHashMap<String, ConcurrentHashMap<String, AtomicInteger>> endpointCounts =
    ...
    public double evaluate(RequestContext ctx) {
        ...
        ConcurrentHashMap<String, AtomicInteger> userEndpoints =
                endpointCounts.computeIfAbsent(userId, k -> new ConcurrentHashMap<>());
